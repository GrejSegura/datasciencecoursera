...
server <- shinyServer(function(input, output){
                        ...
                        if(numberWords == 0){
                                ...
                                for (i in 1:numberWords){
                                        ...
                                        if (nrow(scoreTable) < 5){
                                                ...
                                                predictTable <- rbind(predictTable, pred)
                                        }
                                        pred <- scoreTable[k, c('nextWord')]
                                        predictTable <- rbind(predictTable, pred)
                                }
                ...
                output$predict1 <- renderText({as.character(unlist(predictTable[1,]))})
                output$predict2 <- renderText({as.character(unlist(predictTable[2,]))})
                output$predict3 <- renderText({as.character(unlist(predictTable[3,]))})
                output$predict4 <- renderText({as.character(unlist(predictTable[4,]))})
                output$predict5 <- renderText({as.character(unlist(predictTable[5,]))})
})                        
